...
# the bindings use uint16. Hopefully this won't cause problems.]
...
proc keyCurrState*(): uint16 {.importc: "key_curr_state", header: "tonc.h".}
  ## Get current keystate

proc keyPrevState*(): uint16 {.importc: "key_prev_state", header: "tonc.h".}
  ## Get previous key state

proc keyIsDown*(key: uint16): uint16 {.importc: "key_is_down", header: "tonc.h".}
  ## Gives the keys of ``key`` that are currently down

proc keyIsUp*(key: uint16): uint16 {.importc: "key_is_up", header: "tonc.h".}
  ## Gives the keys of ``key`` that are currently up

proc keyWasDown*(key: uint16): uint16 {.importc: "key_was_down", header: "tonc.h".}
  ## Gives the keys of ``key`` that were previously down

proc keyWasUp*(key: uint16): uint16 {.importc: "key_was_up", header: "tonc.h".}
...
proc keyTransit*(key: uint16): uint16 {.importc: "key_transit", header: "tonc.h".}
  ## Gives the keys of ``key`` that are different from before

proc keyHeld*(key: uint16): uint16 {.importc: "key_held", header: "tonc.h".}
  ## Gives the keys of ``key`` that are being held down

proc keyHit*(key: uint16): uint16 {.importc: "key_hit", header: "tonc.h".}
  ## Gives the keys of ``key`` that are pressed (down now but not before)

proc keyReleased*(key: uint16): uint16 {.importc: "key_released", header: "tonc.h".}
...
proc keyTriHorz*(): int32 {.importc: "key_tri_horz", header: "tonc.h".}
  ## Horizontal tribool (right,left)=(+,-)

proc keyTriVert*(): int32 {.importc: "key_tri_vert", header: "tonc.h".}
  ## Vertical tribool (down,up)=(+,-)

proc keyTriShoulder*(): int32 {.importc: "key_tri_shoulder", header: "tonc.h".}
  ## Shoulder-button tribool (R,L)=(+,-)

proc keyTriFire*(): int32 {.importc: "key_tri_fire", header: "tonc.h".}
...
proc keyRepeat*(keys: uint16): uint16 {.importc: "key_repeat", header: "tonc.h".}
  ## Get status of repeated keys.
  
proc keyRepeatMask*(mask: uint16) {.importc: "key_repeat_mask", header: "tonc.h".}
...
proc keyWaitTillHit*(key: uint16) {.importc: "key_wait_till_hit", header: "tonc.h".}

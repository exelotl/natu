/*
 * strtoll implementation for GBA
 *
 ...
 */
		...
		.section .text.f.strtoll
		.thumb

@ long long strtoll(const char *restrict n, char **restrict endp, int base);

atoll:		movs	r1,0
		movs	r2,10
		@ fall through

strtoimax:	@ alias of strtoll
strtoll:	ldr	r3,=acsl_numParse+1
		mov	r12,r3
		movs	r3,acsl_strtollFunc+1-(.+6)
		add	r3,pc
		bx	r12
...
chkMore:	movs	r3,r6
		cmn	r0,r4		@ add 1 if negative; OK if it
		adcs	r3,r4		@ was 0x80000000 and now 0x7FFFFFFF;
		bpl	1b		@ otherwise fall through to overflow
...
overflow:	ldr	r1,=errno
		...
		lsrs	r6,1		@ shift R0 to get 0x7FFFFFFF
		subs	r0,r4		@ if negative, set it to INT_MIN
		sbcs	r6,r4
		b	1b
